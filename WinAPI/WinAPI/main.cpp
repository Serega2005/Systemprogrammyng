...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!RegisterWindowClass(WndProc, hInstance))
	{
		...
		FormatMessage(
			...
			| FORMAT_MESSAGE_ALLOCATE_BUFFER //Выделяет память для буфера, в котором будет храниться сообщение об ошибке
		...
		MessageBox(NULL, lpBuffer, "Error after RegisterWondowClass function", MB_OK | MB_ICONERROR);
...
ATOM RegisterWindowClass(WNDPROC WndProc, HINSTANCE hInstance)
{
	...
	wc.hIconSm = (HICON)LoadImage(NULL, "Network.ico", IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);//Small icon
...
HWND CreateWnd(HINSTANCE hInstance)
{
	...
	int screen_height = GetSystemMetrics(SM_CYSCREEN);//Получаем разрешение экрана по высоте
	...
	(
		...
		WS_OVERLAPPEDWINDOW,//Стиль окна WS_OVERLAPPWINDOW - это главное окно приложения
		screen_width / 8, screen_height / 8,//Начальная позиция, эти пиксели определяют положение верхнего левого угла
		window_width, window_height, //Размер окна в пикселях
		NULL, //Pодительское окно
		NULL, //Menu
		hInstance,
		NULL
	);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
	{
		...
		if (answer == IDYES)	DestroyWindow(hwnd);//Деактивирует окно  и убирает фокус с указанного окна
		//Фокус - часть окна, которое прнинимает команды с клавиатуры
		break;
	}
	case WM_DESTROY:
